pipeline{
	...
	environment {
		IMAGE_NAME="hdavid0510/chzzk-chat"
	...
	stages {
		...
		stage('Push multiarch image') {
			steps {
				...
				sh 'docker buildx build --push --platform linux/386,linux/amd64,linux/arm/v7,linux/arm64,linux/ppc64le,linux/s390x,linux/riscv64 -t $IMAGE_NAME:$IMAGE_TAG .'
			}
		}
	}
	post {
